...
/* Заголовки разделов «Общих условий» (1. Акцепт, 2. Предмет, и т.д.) —
   bold-параграфы. break-inside:avoid не даёт оторвать заголовок от первого
   пункта при переходе между колонками. Применяется и на screen, и в print. */
.terms-heading{break-inside:avoid;page-break-inside:avoid}
/* v90 (26.06.2026): компактные параграфы в «Общих условиях» — уменьшен
   margin между пунктами 1.1/1.2/... чтобы 8 разделов помещались на одной
   странице вместе с подписью. Раньше подпись выезжала на стр. 3. */
.offer-terms p{margin:1px 0 2px}
@media print{
  body{margin:0}
...
${(() => {
  // v90 (26.06.2026): Жиба «хочу чтобы во всех счетах была такая шапка из
  // лого, адреса и слогана — и в счёт-договорах, и в счёт-офертах, и в
  // обычных». Раньше шапка была только у оферты + текстовый ARTEVO.
  // Теперь рисуется всегда; если в supplier_settings загружен logo_url
  // (через /settings → Реквизиты поставщика → Логотип) — показывается
  // картинка, иначе fallback на текстовый ARTEVO. Контакты тянем из
  // supplier_settings (email/phone), слоган захардкожен как фирменный.
  const logoSrc = supplier?.logo_url ? proxyUrl(supplier.logo_url) : "";
  const sPhone = supplier?.phone ?? "+7 (843) 297-33-22";
  const sEmail = supplier?.email ?? "info@art-evo.ru";
  return `<div style="display:flex;align-items:center;gap:24px;padding-bottom:12px;margin-bottom:14px;border-bottom:1px solid #ddd">
  <div style="flex-shrink:0;width:170px;display:flex;align-items:center;justify-content:flex-start">
    ${logoSrc
      ? `<img src="${logoSrc}" alt="ARTEVO" style="max-width:170px;max-height:60px;object-fit:contain" />`
...
${isOffer ? `<div style="page-break-before:always;font-size:9px;line-height:1.3">
<h3 style="font-size:13px;font-weight:bold;margin:0 0 10px;text-align:center">ОБЩИЕ УСЛОВИЯ ПОСТАВКИ ПО СЧЕТУ-ОФЕРТЕ</h3>

<!-- 2 колонки с column-fill:auto — заполняются последовательно (левая до
     низа, потом правая), без браузерной балансировки. .offer-terms даёт
     компактные margins внутри (margin:1px) — иначе подпись выезжает на
     отдельную страницу (Жиба 26.06.2026). -->
<div class="offer-terms" style="column-count:2;column-gap:20px;column-fill:auto;column-rule:1px solid #eee">
<p class="terms-heading" style="font-weight:bold;margin:0 0 1px">1. Акцепт и заключение договора</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">2. Предмет поставки</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">3. Цена и порядок расчетов</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">4. Порядок поставки и переход права собственности</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">5. Приемка, качество, претензии</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">6. Документы и гарантия</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">7. Условия использования флаконов, емкостей, дозаторов и тары Havenberg / Artevo</p>
...
<p class="terms-heading" style="font-weight:bold;margin:4px 0 1px">8. Ответственность сторон</p>
...
<div style="margin-top:10px;border-top:1px solid #000;padding-top:6px;font-size:10px;position:relative;min-height:110px;break-inside:avoid;page-break-inside:avoid">
${stampSrc ? `<img src="${stampSrc}" style="position:absolute;left:0;top:10px;width:100px;height:100px;opacity:0.8" />` : ""}
${sigSrc ? `<img src="${sigSrc}" style="position:absolute;left:110px;top:4px;width:80px;opacity:0.85" />` : ""}
