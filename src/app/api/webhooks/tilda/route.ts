...
const KNOWN_MESSAGE_KEYS = ["Message", "message", "Сообщение", "comment", "Комментарий", "comments", "Comments", "Cart", "tildacopy"];
...
function parseTildaBlob(text: string): TildaParsed | null {
  ...
  // Жиба 15.05: «The order is paid for.» и «Доставка СДЭК…» шли БЕЗ
  // двоеточия сразу после ключа, поэтому раньше lookahead не находил
  // их и атрибуты product 3 раздувались до Shipping address: с
  // мусором про доставку. Делаем терминаторы без обязательного `:`.
  const productRe = /(\d+)\.\s+(.+?):\s*([\d\s.,]+)\s*\((\d+)\s*x\s*([\d\s.,]+)\)\s*([A-Za-z0-9_\-/]+)?(?:,\s*([^]*?))?(?=\s+\d+\.\s|\s+The\s+order\b|\s+Доставка\b|\s+(?:Shipping\s+address|Full\s+name|Payment|Purchaser|Additional|Block|Transaction|Form\s+Name)\s*:|$)/g;
  ...
  while ((m = productRe.exec(t)) !== null) {
    ...
    // Defensive trim: если регекс терминаторов всё же пропустил
    // «The order is paid for.» или «Доставка СДЭК», вырубаем хвост
    // attributes по этим маркерам — чтобы у позиции не висел мусор
    // про доставку и оплату.
    let attrs = (m[7] || "").trim();
    if (attrs) {
      attrs = attrs.split(/\s+The\s+order\b|\s+Доставка\b/i)[0].trim();
...
export async function POST(req: NextRequest) {
  ...
  for (const candidate of blobCandidates) {
    ...
    if (parsed) {
      ...
      // в КОММЕНТАРИЙ, чтобы не дублировать. Чистим если блоб = messageRaw
      // ИЛИ если messageRaw сам похож на блоб (Tilda иногда шлёт его в
      // алиасе `Comments`/`Cart` которого нет в KNOWN_MESSAGE_KEYS, но
      // тогда тот же текст всё равно дублируется через generic длинные
      // поля).
      if (candidate === messageRaw) cleanMessage = "";
      else if (cleanMessage && /Order\s*#\s*\d+|Full\s+name\s*:|Payment\s+Amount\s*:/i.test(cleanMessage)) {
        cleanMessage = "";
      }
      break;
    }
  ...
  if (resolved.length > 0) {
    // «Объём: 1000мл, Аромат: Grapefruit, basil & pepper» → пары для
    // компактного отображения. Запятая внутри значения (Grapefruit,
    // basil & pepper) не должна резать пару — поэтому split идёт по
    // регексу следующего «Слово:», а не просто по «,».
    function parseAttrPairs(s: string): string {
      const trimmed = s.trim();
      if (!trimmed) return "";
      const pairs: string[] = [];
      const re = /([A-ZА-ЯЁa-zа-яё][A-ZА-ЯЁa-zа-яё /]*):\s*/g;
      const matches: Array<{ key: string; start: number; valStart: number }> = [];
      let mm: RegExpExecArray | null;
      while ((mm = re.exec(trimmed)) !== null) {
        matches.push({ key: mm[1].trim(), start: mm.index, valStart: mm.index + mm[0].length });
      }
      if (matches.length === 0) return trimmed;
      for (let i = 0; i < matches.length; i++) {
        const end = i + 1 < matches.length ? matches[i + 1].start : trimmed.length;
        const value = trimmed.slice(matches[i].valStart, end).replace(/[,\s]+$/, "").trim();
        if (value) pairs.push(`${matches[i].key}: ${value}`);
      }
      return pairs.join(" · ");
    }
