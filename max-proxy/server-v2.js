...
const srv = http.createServer(async (req, res) => {
  ...
  if (u.pathname === "/chats") {
    ...
    const e = cachedChats.map(c => {
      ...
      // Calculate the other user ID via XOR (chatId = viewerId XOR userId)
      const otherUserId = userId ? Number(BigInt(userId) ^ BigInt(cid)) : null;
      const contact = otherUserId ? contactMap.get(String(otherUserId)) : null;
